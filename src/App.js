...
class App extends Component {
  ...
  onClickTab = item => {
    this.setState({selectedTab: item.tabId})
  }

  onClickImg = item => {
    const {displayedImage} = this.state
    if (item.id === displayedImage.id) {
      const randomIndex: number = Math.floor(Math.random() * imagesList.length)

      this.setState(prevState => ({
        displayedImage: imagesList[randomIndex],
  ...
  render() {
    const {selectedTab, displayedImage, time, score} = this.state
    const isTimerZero = time <= 0
    ...
    return (
      <div>
        ...
        {isTimerZero && (
          <div className="scoreBgContainer">
            <img
